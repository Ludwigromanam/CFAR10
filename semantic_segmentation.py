...
def inference(images):
    ...
    fc1_weight = tf.Variable(
        tf.truncated_normal([image_size / (2 * 2 * 2), image_size / (2 * 2 * 2), convdepth3, fcdepth],
                            stddev=0.05))
...
read_image = cv2.imread('/Users/pspitler3/Documents/caffe_images/dogandcat.jpg')
# image = image[40:320, :, :]
read_image = cv2.resize(read_image, (new_image_size, new_image_size), interpolation=cv2.INTER_AREA)
imdata = cv2.cvtColor(read_image, cv2.COLOR_BGR2RGB)
